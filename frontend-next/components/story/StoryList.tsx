...
export default function StoryList({ groups }: StoryListProps) {
  ...
  return (
    <section
      aria-label="Stories"
      className="w-full bg-white py-4 sm:rounded-xl"
    >
          ...
          <div className="relative h-[min(78vh,680px)] w-full max-w-97.5 overflow-hidden rounded-2xl bg-slate-900 shadow-2xl">
            ...
            <div className="absolute left-3 right-3 top-3 z-10 flex gap-1">
              {activeGroup.stories.map((story, index) => (
                <span
                  className="h-1 flex-1 rounded-full bg-white/40"
                  key={story.story_id}
                >
                  <span
                    className={`block h-full rounded-full bg-white ${index <= activeIndex ? "w-full" : "w-0"}`}
                  />
                </span>
              ))}
            </div>
            ...
            <div className="absolute left-4 top-8 z-10 flex items-center gap-2 text-white">
              <div className="relative h-8 w-8">
                <Image
                  ...
                  fill
                  src={assetUrl(
                    activeGroup.author?.avatar_url || activeGroup.author?.avata,
                  )}
                />
              </div>
            ...
            {activeMedia ? (
              <div className="relative h-full w-full">
                <Image
                  ...
                  src={assetUrl(activeMedia)}
                />
              </div>
            ) : (
              <div
                className="flex h-full items-center justify-center p-10 text-center text-2xl font-semibold text-slate-800"
                style={{
                  backgroundColor: activeStory.background_color || "#FFE1D6",
                }}
              >
                {activeStory.content}
              </div>
